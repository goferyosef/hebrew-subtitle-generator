...
Handles:
  - Plain multi-line subtitles (newline-separated in the SRT block)
  - <br /> wrapped long lines (as used by Subtitle Edit and similar tools)
"""
...
# Placeholder used internally so we can send each subtitle as a single line
# to the API without confusing it with SRT structure.
BR_PLACEHOLDER = "<<BR>>"
...
def parse_srt(content):
    """Parse SRT content into a list of {idx, timing, text} dicts.
    Preserves <br /> tags and real newlines inside subtitle text."""
...
def text_to_single_line(text):
    """Convert subtitle text to a single line for the API prompt.
    Both real newlines and <br /> become the internal BR_PLACEHOLDER."""
    # normalise <br /> variants: <br/>, <BR />, <BR/> etc.
    text = re.sub(r"<br\s*/?>", BR_PLACEHOLDER, text, flags=re.IGNORECASE)
    # also turn real newlines into the placeholder
    text = text.replace("\n", BR_PLACEHOLDER)
    return text


def single_line_to_text(line, original_text):
    """Restore line breaks in the translated result.
    Decides whether to use <br /> or \n based on what the original used."""
    uses_br = bool(re.search(r"<br\s*/?>", original_text, re.IGNORECASE))
    if uses_br:
        return line.replace(BR_PLACEHOLDER, "<br />")
    else:
        return line.replace(BR_PLACEHOLDER, "\n")
...
def translate_batch(api_key, texts):
    """Translate a list of subtitle strings to Hebrew.
    Each text is flattened to one line using BR_PLACEHOLDER before sending."""

    # flatten each subtitle to a single line
    flat_texts = [text_to_single_line(t) for t in texts]

    numbered = "\n".join(f"[{i+1}] {t}" for i, t in enumerate(flat_texts))

    prompt = (
        "Translate the following subtitle lines to Hebrew.\n"
        "Rules:\n"
        f"- Each subtitle is on ONE line, prefixed with [N].\n"
        f"- The token {BR_PLACEHOLDER} marks a line break inside a subtitle. "
        f"Keep it in the translation at a natural break point.\n"
        "- Return ONLY the translations, each prefixed with [N] where N is the number.\n"
        "- Every translation must be on exactly ONE line.\n"
        "- Do NOT add any explanation, blank lines between entries, or extra text.\n\n"
        + numbered
    )
    ...
    for line in result.split("\n"):
        ...
        if m:
            ...
            # restore line breaks using the original text as reference
            original   = texts[idx - 1] if 0 < idx <= len(texts) else ""
            out[idx]   = single_line_to_text(translated, original)
    return out
